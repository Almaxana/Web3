...
func (s *Server) proceedMainTxMakeBet(ctx context.Context, nAct *notary.Actor, notaryEvent *result.NotaryRequestEvent, bet int) error {
...
func validateNotaryRequestMakeBet(req *payload.P2PNotaryRequest) (util.Uint160, int, error) {
	...
	if !bytes.Equal(ops[opsLen-1].param, contractSysCall) {
		return util.Uint160{}, 0, errors.New("not contract syscall")
	}
