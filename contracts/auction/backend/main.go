	...
	for {
		...
		case notaryEvent, ok := <-s.sub.NotificationChannels().NotaryRequestsCh: // ждем события из канала NotaryRequestsCh,
			...
			case mempoolevent.TransactionAdded:
				tokenName, _, _, err := s.parseNotaryEvent(notaryEvent)
				if err != nil {
...
func validateNotaryRequest(req *payload.P2PNotaryRequest, s *Server) (util.Uint160, string, []byte, int, error) {
	...
	case "start":
		sh, nftIdBytes, initBet, err = validateNotaryRequestStartAuction(req, s)
	case "makeBet":
		sh, initBet, err = validateNotaryRequestMakeBet(req, s)
	case "finish":
...
func validateNotaryRequestPerProcessing(req *payload.P2PNotaryRequest) ([]Op, util.Uint160, error) {
	var (
